...
As noted earlier, HCM control delay and VISSIM queue delay 
...
Both HCM and VISSIM assign LOS E to the Anamnagar approach, 
...
Given the fundamental differences between HCM control delay and 
...
of traffic flow with excessive delays. 
...
The peak hour LOS comparison at Babarmahal Intersection tells a 
clear story — the existing signal timing is struggling, and only 
the manually optimized Scenario 3 delivers meaningful system-wide 
relief. The most significant headline finding is the overall 
intersection LOS improving from LOS E under both Scenarios 1 and 2 
to LOS D under Scenario 3, confirming that the manually optimized 
timing plan is the only one capable of upgrading the intersection's 
overall service grade. The Maitighar Main to Art Gallery movement 
remains at LOS F across all three scenarios, indicating a persistent 
demand-capacity problem on this movement that fixed-time 
optimization alone cannot resolve. Encouragingly, Maitighar Main to 
Babarmahal Main, Babarmahal Service to Art Gallery, Babarmahal 
Service to Maitighar Service, and Anamnagar to Babarmahal Service 
all improve to LOS C under Scenario 3, representing the most 
impactful gains of the study. A noteworthy concern is the Anamnagar 
to Art Gallery movement, which actually worsens from LOS E to LOS F 
under Scenario 2 before recovering to LOS E under Scenario 3, 
exposing the limitations of the VISSIM-optimized plan. Minor 
trade-offs are observed at the Art Gallery to Babarmahal Main and 
Art Gallery to Maitighar Main movements, which slip from LOS D to 
LOS E under Scenario 3, reflecting a slight redistribution of green 
time away from this approach. Overall, Scenario 3 improves nine out 
of seventeen movements compared to the baseline, achieving the most 
balanced and effective LOS distribution across the Babarmahal 
