...
and orderly traffic flow. (Duraku \& Boshnjaku, 2024). Traffic 
...
setting device for the specific intersection (Fwha,2008). Traffic 
...
public transportation, and motorbikes (Eom \& Kim, 2020). Traffic s
ignals are electrically operated control devices to avoid conflicts 
between vehicles and pedestrians (Ryder Richardson et al., 2005). The 
...
multiple lanes receiving the right of way simultaneously (HCM, 2000). 
...
using data from detectors (Duraku \& Boshnjaku, 2024)
...
signal cycle regardless of real-time traffic demand. (FHA,2007). 
...
a maximum of three phases is required (Urbanik et al., 2015). In 
...
delay, fuel consumption, and exhaust emissions (Grether et al.). 
Changing the timing of traffic signals is a cheap and simple way 
to improve traffic flow and reduce congestion. (FHA, 2007). But 
...
especially during peak hours (Costa et al.,2018).
...
might decrease or can be adversely affected (Grether et al., 2011). 
...
in the intersection approaches. (Akcelik, 1994). One of the applications 
...
effectiveness and functions of the actuated systems. (Cakici et al., 2021). 
...
(Ravikumar, 2012). An ATCS is a complex system that involves 
...
reducing the number of stops (Stevanovic, 2010). The analysis 
...
(Cakici et al., 2021). The actuated signal controller has detectors 
...
(Urbanik et al., 2015). The maximum green time should be set for 
...
performance (Duraku \& Boshnjaku, 2024). Vehicle-actuated signal systems were 
...
(Fhwa, 2008). Using a fuzzy logic-based vehicle actuated management 
system reduced delays by 10% (Trabia et al). The green time extension 
investigation for vehicle-actuated signal control (Wang et al., 2018). 
...
with control centers(Stevanovic, 2010). Adaptive traffic control 
...
demand or movement(Martin, 2008). Various adaptive traffic control 
...
are built in only a few cities around the world (Tomar et al., 2022).  