...
it will affect the efficiency of the intersection \cite{sinowatcher_actuatedsignals}.
...
multiple intersections \cite{brown2019strategies}.
...
system reduced delays by 10\% \cite{trabia1999}. The green time extension 
...
systems \cite{ribeiro2012fully}. At the end of the analysis, it 
...
demand or movement\cite{stevanovic2008}. Various adaptive traffic control 
...
\section{Traffic Signal Control in Context of Nepal}

Research on traffic efficiency in Nepal has traditionally focused on optimizing static, fixed-time signal cycles. 
Historically, researchers at the Institute of Engineering (IOE) have prioritized determining optimal green splits 
for saturated nodes to maximize capacity. For instance, a performance study at the Satdobato intersection demonstrated 
that optimizing signal parameters, specifically cycle time and phasing, could lead to a substantial reduction in average 
vehicle delays \cite{dhakal2023evaluation}.Similarly, an assessment of the New Plaza intersection in Kathmandu found that simply 
re-timing the existing fixed-signal phases using field-validated data could significantly decrease queue length and average 
delay in the intersection \cite{luitel2025performance}.However, these studies primarily address peak-hour congestion through pre-determined plans, 
leaving the system unable to respond to the high fluctuation of traffic volumes characteristic of Kathmandu's roads. A major study by\cite{bajracharya2023urban}
evaluated pretimed versus actuated signalization in a coordinated network at the Keshar Mahal and Durbar Marg intersections. 
Using SIDRA INTERSECTION, the study found that pretimed coordination with an optimal network cycle was more effective than isolated actuation, 
reducing average control delay by 49.2\%\cite{bajracharya2023urban}.While these results suggest a preference for pretimed systems, the study was conducted 
using lane-based micro-analytical tools that assume disciplined driving behavior. In Kathmandu's heterogeneous, non-lane-based traffic, such lane-based 
models often fail to capture the stochastic reality of vehicle interactions. Furthermore, while the study compared coordination strategies, 
it did not explore semi-actuated control, a logic specifically suited for major-minor intersections like Babarmahal, where minor-street demand 
is intermittent. To date, no research in Nepal has rigorously evaluated semi-actuation in a heterogeneous traffic context.

In December 2024, Lalitpur Metropolitan City introduced Nepal's first real-world adaptive traffic system at five locations, including Pulchowk and Jawalakhel. 
While these lights use vehicle detectors to adjust green times based on live density, there is a lack of academic documentation validating their performance with 
engineering metrics like delay or queue length. To bridge this gap, recent frameworks explore Multi-Agent Reinforcement Learning (MARL). Specifically, utilizing 
Multi-Agent Deep Deterministic Policy Gradient (MADDPG) integrated with Simulation of Urban Mobility (SUMO) and YOLOv5 for vehicle detection has shown that adaptive 
systems can reduce average vehicle delay by 30-35\% and maximum queue length by 25\% compared to fixed-time mechanisms \cite{maharjan2025leveraging}. While SUMO is 
used for high-level RL studies and SIDRA for basic analytical modeling, microscopic simulation using PTV VISSIM paired with the VISVAP algorithm provides a superior 
platform for programming deployable, detector-based logic. in Nepal because it can be calibrated to account for the "non-lane-based" and "heterogeneous" nature of 
Kathmandu's traffic \cite{manandhar2023calibration}. Unlike analytical models, VISSIM models individual vehicle trajectories, which is essential for designing context-sensitive, 
semi-actuated systems that respond to specific vehicle arrivals on minor streets \cite{afshari2025calibrating}. 

\subsection{Operational Principles of Vis-VAP}

Vis-VAP operates by defining actuated signal control logic based on 
detector configurations and phase operation parameters for an 
intersection. The software allows users to configure detector types 
and locations, phase sequences following the NEMA dual-ring 
structure, minimum and maximum green times, passage time extensions, 
gap reduction parameters, and yellow and all-red clearance 
intervals to realistically model actuated controller behavior 
\cite{ptv2022vissim}. Once the control parameters are specified, 
VISVAP generates a Vehicle Actuated Programming (VAP) file that 
can be integrated with PTV VISSIM, where the VAP module dynamically 
controls signal operations in response to simulated vehicle 
detections, enabling realistic evaluation of traffic-responsive 
signal performance\cite{fellendorf2010vissim}.



\subsection{Level of Service (LOS)}
Level of Service (LOS) is a qualitative measure used to evaluate the 
operational performance and quality of traffic flow on a roadway 
segment or at an intersection based on factors such as speed, travel 
time, maneuverability, traffic interruptions, and delay 
\cite{trb2022hcm}. Both the Highway Capacity 
Manual (HCM) and the A Policy on Geometric Design of Highways and 
Streets classify traffic operating conditions into six categories 
ranging from LOS A to LOS F, where LOS A represents free-flow 
conditions with minimal delay and high driving comfort, while LOS F 
indicates heavily congested conditions with unstable flow, excessive 
delay, and demand exceeding roadway capacity \cite{aashto2018geometric}. LOS A, 
B, and C generally represent stable traffic conditions with 
relatively high operating speeds and acceptable maneuverability, 
whereas LOS D and E reflect increasingly unstable operations near 
roadway capacity with reduced driver comfort and higher sensitivity 
to disturbances. LOS F corresponds to breakdown flow conditions 
characterized by stop-and-go movement, long delays, and unreliable 
travel times. LOS is widely used in traffic engineering studies for 
assessing roadway performance, evaluating signalized intersections, 
and identifying the need for operational or geometric improvements.
...
The results of the simulation are compared on the basis of the Geoffrey E. Havers (GEH) statistic as per \cite{analysis_procedures_manual}. 

The GEH formula is given by the equation given below:
...
The calibrated parameters \cite{manandhar2026calibration} are 
presented in the table below and were used as a reference for the 
calibration of VISSIM. The table shows various parameters as listed 
below.
...
\begin{tabular}{|l|l|l|}
    ...
    \hline
    1 &	Look ahead distance-min &	20 m \\
    \hline
    2 &	Look-back distance-min	& 18.71 m \\
    \hline
    3 &	Average standstill distance	& 1.5 m \\
    \hline
    4 &	Additive part of safety distance	& 0.5 \\
    \hline
    5 &	Multiplicative part of safety distance	& 1 \\
    \hline
